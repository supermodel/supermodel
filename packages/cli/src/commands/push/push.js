...
async function runPush() {
  try {
    ...
    console.log(`--> Successfully pushed to supermodel.io`);
  } catch (error) {
    console.error(error);
    process.exit(1);
  }
...
async function push() {
  ...
  if (!process.env['SUPERMODEL_TOKEN'] && (!user || !user.username)) {
    throw new Error(
      `Please login first using 'supermodel login or use token via SUPERMODEL_TOKEN'.`,
    );
  }
  ...
  const supermodelDirectory = supermodelConfig.findSupermodelDir(
    currentDirectory,
  );
  ...
  if (supermodelDirectory) {
    if (supermodelDirectory == currentDirectory) {
      throw new Error(
        `Root supermodel directory '${currentDirectory}' cannot be pushed`,
      );
    }
    ...
    throw new Error(
      `Directory '${currentDirectory}' is not within supermodel scope.`,
    );
  }
}
...
async function updateLayer(layerPath, layerData) {
  ...
  if (!response.ok) {
    ...
    if (response.status === 400) {
      ...
      throw new Error(
        `Push failed for insufficient permissions to '${layerPath}'`,
      );
    } else {
...
function FSModelToEntity(modelFile, errors) {
  ...
  try {
    ...
    addErrorMessage(
      errors,
      modelFile,
      `cannot read or parse model schema. Error: ${error}`,
    );
    return;
  }
    ...
    if (schema.$id) {
      if (!schema.$id.startsWith(process.env['SCHEMA_ORIGIN'])) {
        addErrorMessage(
          errors,
          modelFile,
          `model $id origin is invalid. Valid: ${process.env['SCHEMA_ORIGIN']}`,
        );
      }
      ...
      if (idPath != modelPath) {
        addErrorMessage(
          errors,
          modelFile,
          `model file path does not match its $id '${idPath}'`,
        );
      }
    } else {
